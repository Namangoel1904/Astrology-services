...
export async function POST(req: NextRequest) {
  try {
    ...
    let { dob, tob, lat, lon, tz = 5.5, lang = "hi", planet = "Sun" } =
      body || {};
    ...
    if (!upstream.ok) {
      ...
      console.error("Bhinnashtakvarga upstream error:", upstream.status, errorText);
      return NextResponse.json(
    ...
    console.error("Bhinnashtakvarga API error:", error);
    return NextResponse.json(
