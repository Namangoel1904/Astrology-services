...
function ShaderPlane() {
  ...
  return (
    <mesh ref={meshRef} position={[0, -0.75, -0.5]}>
      ...
      {/* @ts-expect-error three fiber extended element */}
...
export default function Hero({
  ...
  useGSAP(
    () => {
      ...
      // wait for fonts, then split and animate
      // @ts-expect-error fonts may not exist in some environments
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-7 px-6 pb-28 pt-40 sm:gap-9 sm:pt-48 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-2.5 rounded-full border border-white/10 bg-white/5 px-4 py-2 backdrop-blur-sm">
          <span className="text-xs font-light uppercase tracking-[0.1em] text-white/70">{badgeLabel}</span>
          <span className="h-1.5 w-1.5 rounded-full bg-white/45" />
          <span className="text-sm font-light tracking-tight text-white/85">{badgeText}</span>
        </div>

        <h1 ref={headerRef} className="max-w-4xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl">
          {title}
        </h1>

        <p ref={paraRef} className="max-w-2xl text-left text-lg font-light leading-relaxed tracking-tight text-white/80 sm:text-xl">
          {description}
        </p>

        <div ref={ctaRef} className="flex flex-wrap items-center gap-4 pt-3">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border border-white/10 px-6 py-3.5 text-base font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-white/30 duration-300 ${
                button.primary
      ...
      {/* Right-side pocket card (9:16) – metallic, mid-right, floating */}
      <div className="hidden lg:block absolute top-1/2 right-[12%] -translate-y-1/2">
        <div style={{ perspective: 1000 }}>
          ...
          >
            <div
              className="rounded-xl border border-black/30 p-[6px] shadow-2xl"
              style={{
                backgroundImage: "linear-gradient(135deg,#e0e0e0 0%,#434343 100%)",
                boxShadow:
                  "inset 0 0 4px rgba(255,255,255,0.5), inset 0 0 8px rgba(0,0,0,0.4), 0 24px 60px rgba(0,0,0,0.45)",
              }}
            >
              <div className="relative h-[448px] w-72 overflow-hidden rounded-md bg-black/80">
                <Image
