...
export default function PanchangPage() {
  ...
  async function onHoroscopeSubmit(formData: FormData) {
    ...
    console.log("Form data:", { date, zodiac });
  ...
  // API returns { status: 200, response: { ... } }
  const pretty = (result as any)?.response ?? result;
  const horoscope = horoscopeResult?.response;

  return (
    <div className="container mx-auto px-4 py-10 bg-gradient-to-b from-amber-50 via-orange-50 to-amber-100 dark:from-amber-950/30 dark:via-orange-950/20 dark:to-amber-900/20">
      <h1 className="text-3xl font-semibold mb-2 text-amber-900 dark:text-amber-200">पंचांग आणि दैनिक भविष्य</h1>
      <p className="mb-6 text-amber-800/80 dark:text-amber-100/70">दैनिक पंचांग तपशील आणि राशिफल</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="p-6 space-y-6 border-amber-300/60 dark:border-amber-700/60 bg-white/80 dark:bg-amber-950/20 backdrop-blur-sm">
          <h2 className="text-xl font-semibold text-amber-900 dark:text-amber-200">आजचे पंचांग</h2>
          ...
          <form action={onSubmit} className="grid grid-cols-1 gap-4">
            ...
            {/* Hidden language field defaults to Marathi (mr) */}
            <input id="lang" name="lang" defaultValue="mr" hidden readOnly />

            <div>
              <Button type="submit" disabled={loading}>{loading ? "पंचांग मिळवत आहे..." : "पंचांग मिळवा"}</Button>
            </div>
          </form>
        ...
        {/* Horoscope Section */}
        <Card className="p-6 space-y-6 border-amber-300/60 dark:border-amber-700/60 bg-white/80 dark:bg-amber-950/20 backdrop-blur-sm">
          <h2 className="text-xl font-semibold text-amber-900 dark:text-amber-200">दैनिक राशिफल</h2>
          ...
          <form onSubmit={(e) => {
            ...
            <div>
              <Button type="submit" disabled={horoscopeLoading}>{horoscopeLoading ? "राशिफल मिळवत आहे..." : "राशिफल मिळवा"}</Button>
            </div>
          </form>

          {horoscopeError && <div className="text-red-500 text-sm">{horoscopeError}</div>}
          ...
          {horoscope && (
            <div className="space-y-6">
              ...
              <Section title="आजचे भविष्यवाणी">
                <div className="p-4 bg-amber-50/60 dark:bg-amber-900/20 rounded-lg">
